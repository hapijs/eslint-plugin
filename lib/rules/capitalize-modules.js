...
module.exports = {
    ...
    create(context) {
        ...
        const atTopLevel = function (node) {

            return (sourceCode.getAncestors ? sourceCode.getAncestors(node) : context.getAncestors()).every((parent) => internals.validTopLevelParents.has(parent.type));
        };

        const check = function (node) {

            if (globalScopeOnly === true &&
                atTopLevel(node) === false) {
