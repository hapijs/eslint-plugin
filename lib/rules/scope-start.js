...
module.exports = {
    ...
    create(context) {
        ...
        const check = function (node, allowOneLiners) {
            ...
            const openTokenLine = sourceCode.getTokenBefore(stmt).loc.start.line;
            const closeTokenLine = isBlockBody ? sourceCode.getTokenAfter(stmt).loc.start.line : sourceCode.getLastToken(stmt).loc.start.line;
